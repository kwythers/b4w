...
####START WITH PRIMARY15 and PRIMARY60
...
sec<-"/Volumes/disk7/b4warmed3/export/primary2013/primary15c_2013.csv"
sec2<-"/Volumes/disk7/b4warmed3/export/primary2013/primary15d_2013.csv"
sec3<-"/Volumes/disk7/b4warmed3/export/primary2013/primary15e_2013.csv"
sec4<-"/Volumes/disk7/b4warmed3/export/primary2013/primary15_agg_2013.csv"
primary60_dir<-"/Volumes/disk7/b4warmed3/export/primary2013/primary60_2013.csv"
texture<-"/Volumes/disk7/b4warmed3/derived_data/texture.csv"
sec5<-"/Volumes/disk7/b4warmed3/export/primary2013/primary15_agg2_2013.csv"
...
dt4<-dt2[treatment_abbr=="dc"& water_treatment_abbr=="ambient",]
...
dt5<-unique(subset(dt2, select=col_keep))
...
write.table(dt4,sec4, append = FALSE, quote = FALSE, sep = ",",
           , na = "NA", dec = ".", row.names = FALSE,
          col.names = TRUE, qmethod = c("escape", "double"))
...
cols4<- c("year","hour","doy","site", "canopy","p_above_meanf2_dc_new3","p_above_meanf2_dc2_new3","d_tsoilf2_dc_new2","d_tsoilf2_dc2_new2")
...
ky<-c("plot","month","hour","year")
dvwc2[,d_tsoil_sc_mm:= mean(d_tsoilf2_sc_new3, na.rm=TRUE),by = ky]
# dvwc2[,d_tsoil_delta_sc_mm:= mean(d_tsoilf2_delta, na.rm=TRUE),by = ky]
dvwc2[,d_tabove_sc_mm:= mean(p_tabovef2_sc_new3, na.rm=TRUE),by = ky]
# dvwc2[,d_tabove_delta_sc_mm:= mean(p_tabovef2_delta, na.rm=TRUE),by = ky]
...
dt2[d_period_omit == 0, d_periodf:= NA] 
...
dvwc[,d_period_tscor:= d_periodf2+(20-d_tsoilf2_sc)*(0.526-0.052*d_periodf2+0.00136*d_periodf2^2)]
dvwc[is.na(d_period_tscor), d_period_tscor:= d_periodf2+(20-d_tsoil_sc_mm)*(0.526-0.052*d_periodf2+0.00136*d_periodf2^2)]
...
drain[,d_rain:= d_rain/2]
setkey(drain,site,time2)
setkey(dvwc2,site,time2)
dvwc2<-merge(drain, dvwc2)
